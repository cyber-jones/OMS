...
const DetailedSpecialty = () => {
    ...
    const getSpecialty = async () => {
        setLoading(true);
        try {
            const res = await axioAnonymous(oms_server_dev_url.doctor).get("/specialty/"+id);
            if (res?.status !== 200) 
  ...
  return (
    <>
        {!loading && specialty ? 
            <div className='w-[50%] p-20 bg-gray-300 rounded-lg shadow-lg'>
                <h1 className='text-3xl font-bold mb-6'>{specialty?.name}</h1>
